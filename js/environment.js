...
class Environment {
    ...
    createFloor() {
        // Create floor with PBR materials and texture
        const floorGeometry = new THREE.PlaneGeometry(20, 20);
        
        // Create a texture for the floor - a checkerboard pattern
        const floorTexture = this.createCheckerboardTexture(0x606060, 0x707070, 32);
        
        const floorMaterial = new THREE.MeshStandardMaterial({
            color: 0x90A4AE, // Bluish-gray floor
            roughness: 0.8,
            metalness: 0.2,
            map: floorTexture
        });
    ...
    createWalls() {
        // Create walls with more vibrant color and texture for contrast
        const wallMaterial = new THREE.MeshStandardMaterial({
            color: 0xDCE775, // Light greenish-yellow wall
            roughness: 0.9,
            metalness: 0.1
        });
        
        // Add a baseboard trim for better visual distinction
        const baseboardMaterial = new THREE.MeshStandardMaterial({
            color: 0x5D4037, // Dark brown baseboard
            roughness: 0.7,
            metalness: 0.3
        });
        ...
        // Back wall baseboard
        const backBaseboardGeometry = new THREE.BoxGeometry(20, 0.2, 0.1);
        const backBaseboard = new THREE.Mesh(backBaseboardGeometry, baseboardMaterial);
        backBaseboard.position.set(0, 0.1, -9.95);
        ...
        // Left wall baseboard
        const leftBaseboardGeometry = new THREE.BoxGeometry(20, 0.2, 0.1);
        const leftBaseboard = new THREE.Mesh(leftBaseboardGeometry, baseboardMaterial);
        leftBaseboard.position.set(-9.95, 0.1, 0);
        ...
        const rightBaseboardGeometry = new THREE.BoxGeometry(20, 0.2, 0.1);
        const rightBaseboard = new THREE.Mesh(rightBaseboardGeometry, baseboardMaterial);
        rightBaseboard.position.set(9.95, 0.1, 0);
        ...
        // Front left wall baseboard
        const frontLeftBaseboardGeometry = new THREE.BoxGeometry(8, 0.2, 0.1);
        const frontLeftBaseboard = new THREE.Mesh(frontLeftBaseboardGeometry, baseboardMaterial);
        frontLeftBaseboard.position.set(-6, 0.1, 9.95);
        frontLeftBaseboard.castShadow = true;
        this.scene.add(frontLeftBaseboard);
        ...
        // Front right wall baseboard
        const frontRightBaseboardGeometry = new THREE.BoxGeometry(8, 0.2, 0.1);
        const frontRightBaseboard = new THREE.Mesh(frontRightBaseboardGeometry, baseboardMaterial);
        frontRightBaseboard.position.set(6, 0.1, 9.95);
        frontRightBaseboard.castShadow = true;
        this.scene.add(frontRightBaseboard);
    ...
    // Helper method to create a checkerboard texture
    createCheckerboardTexture(color1, color2, size) {
        ...
        for (let x = 0; x < 2; x++) {
            for (let y = 0; y < 2; y++) {
                context.fillStyle = (x + y) % 2 === 0 ? '#' + color1.toString(16).padStart(6, '0') : '#' + color2.toString(16).padStart(6, '0');
        ...
        texture.repeat.set(5, 5); // Repeat the texture 5 times on each axis
